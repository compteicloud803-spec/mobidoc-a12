...
// Try iOS-specific plist first, then fallback to generic
$plistPath = null;
if (!empty($ios)) {
    $iosPath = "$basePath/Maker/$prdFormatted/$ios/com.apple.MobileGestalt.plist";
    log_debug("Trying iOS-specific plist: $iosPath");
    if (file_exists($iosPath)) {
        $plistPath = $iosPath;
    }
}
if (!$plistPath) {
    $genericPath = "$basePath/Maker/$prdFormatted/com.apple.MobileGestalt.plist";
    log_debug("Trying generic plist: $genericPath");
    if (file_exists($genericPath)) {
        $plistPath = $genericPath;
    }
}
if (!$plistPath) {
    log_debug("Plist not found for prd='$prdFormatted' ios='$ios'", "ERROR");
